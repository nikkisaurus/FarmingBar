...
function addon:PreviewAlert(alertType, input, info)
	...
	elseif alertType == "button" then
		...
		alertInfo = {
			...
			objective = {
				color = (objective and objective > 0) and (newCount >= objective and "|cff00ff00" or "|cffffcc00")
					or "",
				count = objective,
			},
			oldCount = oldCount,
...
function addon:SendAlert(bar, alertType, alert, alertInfo, soundID, isTracker)
	...
	local newCompletion = (alertInfo.objective.count or 0)
		and (alertInfo.newCount or 0) > (alertInfo.objective.count or 0)
		and (alertInfo.oldCount or 0) < (alertInfo.objective.count or 0)
