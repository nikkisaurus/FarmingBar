...
function private:GetTrackerCount(trackerInfo, bar, buttonID, overrideObjective)
    ...
    if trackerInfo.type == "ITEM" then
        if DS then
            ...
            count = floor(GetItemCount(trackerInfo.id, trackerInfo.includeBank, _, trackerInfo.includeBank) / objective)
        end
    ...
    for _, altInfo in pairs(trackerInfo.altIDs) do
        ...
        if altInfo.type == "ITEM" then
            if DS then
                ...
                altCount = floor(GetItemCount(altInfo.id, trackerInfo.includeBank, _, trackerInfo.includeBank) * altInfo.multiplier)
            end
