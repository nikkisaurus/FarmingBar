...
function addon:GetButtonTooltip(widget, tooltip)
    ...
    if self:GetDBValue("global", "settings.hints.buttons") then
        ...
        if self:IsTooltipMod() then
            ...
            tooltip:AddDoubleLine(L["Expand Tooltip"] .. ":", L[self:GetDBValue("global", "settings.tooltips.modifier")], unpack(self.tooltip_keyvalue))
        end
    end
end
...
--@retail@
                ...
                if data.conditionInfo == "" then
                    tooltip:AddDoubleLine(data.name,
                        format("%s%d%s|r (%d / %d)", ((objective and objective > 0) and (trackerCount < objective)) and utils.ChatColors["YELLOW"] or utils.ChatColors["GREEN"], trackerCount, (objective and objective > 0) and " / " .. objective or "",
                            trackerRawCount, ((trackerInfo.objective and trackerInfo.objective > 0) and trackerInfo.objective or 1) * ((objective and objective > 0) and objective or 1)), unpack(self.tooltip_description))
                else
