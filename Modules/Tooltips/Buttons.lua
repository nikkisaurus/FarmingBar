...
function addon:GetButtonTooltip(widget, tooltip)
    ...
    if not widget:IsEmpty() then
        ...
        if buttonDB.action and buttonDB.actionInfo and (buttonDB.action == "ITEM" or buttonDB.action == "CURRENCY") then
            ...
            GameTooltip_AddBlankLinesToTooltip(FarmingBar_Tooltip, 1)
            FarmingBar_Tooltip:AddTexture(389194, {
                width = 200,
                height = 10
            })
        end
    ...
    if self:GetDBValue("global", "settings.hints.buttons") then
        ...
        if self:IsTooltipMod() then
            ...
            if widget:IsEmpty() then
                FarmingBar_Tooltip:AddLine(L.ButtonHints("showQuickAddEditBox", self:GetDBValue("global",
                    "settings.keybinds.button.showQuickAddEditBox")), unpack(self.tooltip_description))
                -- @retail@
                FarmingBar_Tooltip:AddLine(L.ButtonHints("showQuickAddCurrencyEditBox", self:GetDBValue("global",
                    "settings.keybinds.button.showQuickAddCurrencyEditBox")), unpack(self.tooltip_description))
                -- @end-retail@
            else
...
function addon:GetButtonObjectiveInfo(widget, tooltip, buttonDB)
    ...
    if buttonDB.action and buttonDB.actionInfo then
        if buttonDB.action == "MACROTEXT" then -- MACROTEXT
            tooltip:AddDoubleLine(L["Action"], strsub(buttonDB.actionInfo, 1, 15) ..
                (strlen(buttonDB.actionInfo) > 15 and "..." or ""), unpack(self.tooltip_keyvalue))
        else
            self:GetTrackerDataTable(buttonDB, buttonDB.action, buttonDB.actionInfo, function(data) -- OTHERS
                tooltip:AddDoubleLine(L["Action"], strsub(data.name, 1, 15) .. (strlen(data.name) > 15 and "..." or ""),
                    unpack(self.tooltip_keyvalue))
            end)
        end
    else -- NONE
    ...
    -- Tracker condition
    tooltip:AddDoubleLine(L["Condition"],
        L[strsub(buttonDB.condition, 1, 1) .. strlower(strsub(buttonDB.condition, 2))], unpack(self.tooltip_keyvalue))
    ...
    if numTrackers == 1 then
        tooltip:AddDoubleLine(L["Include All Characters"], buttonDB.trackers[self:GetFirstTracker(widget)]
            .includeAllChars and L["TRUE"] or L["FALSE"], unpack(self.tooltip_description))
        tooltip:AddDoubleLine(L["Include Bank"],
            buttonDB.trackers[self:GetFirstTracker(widget)].includeBank and L["TRUE"] or L["FALSE"],
            unpack(self.tooltip_description))
        -- @retail@
        tooltip:AddDoubleLine(L["Include Guild Bank"],
    ...
    tooltip:AddDoubleLine(L["Objective"], (objective and objective > 0) and objective or L["FALSE"],
        unpack(self.tooltip_keyvalue))
    if objective and objective > 0 then
        tooltip:AddDoubleLine(L["Objective Complete"],
            count >= objective and floor(count / objective) .. "x" or L["FALSE"], unpack(self.tooltip_description))
    end
    ...
    for key, trackerInfo in pairs(buttonDB.trackers) do
        ...
        if trackerCount > self.maxTooltipTrackers then -- Shorten list
            tooltip:AddLine(format("%d %s...", numTrackers - self.maxTooltipTrackers, L["more"]),
                unpack(self.tooltip_description))
            ...
            self:GetTrackerDataTable(buttonDB, trackerType, trackerID, function(data)
                ...
                if trackerType == "ITEM" then
                    -- @retail@
                    trackerRawCount = (trackerInfo.includeAllChars or trackerInfo.includeGuildBank) and
                ...
                elseif trackerType == "CURRENCY" and trackerID ~= "" then
                    trackerRawCount = trackerInfo.includeAllChars and
                                          self:GetDataStoreCurrencyCount(trackerID, trackerInfo) or
                ...
                if data.conditionInfo == "" then
                    tooltip:AddDoubleLine(data.name, format("%s%d%s|r (%d / %d)",
                        ((objective and objective > 0) and (trackerCount < objective)) and utils.ChatColors["YELLOW"] or
                            utils.ChatColors["GREEN"], trackerCount, (objective and objective > 0) and " / " ..
                            objective or "", trackerRawCount,
                        ((trackerInfo.objective and trackerInfo.objective > 0) and trackerInfo.objective or 1) *
                            ((objective and objective > 0) and objective or 1)), unpack(self.tooltip_description))
                else
