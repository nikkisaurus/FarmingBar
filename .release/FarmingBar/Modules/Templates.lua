...
function addon:LoadTemplate(templateType, barID, templateName, withData, saveOrder)
	local template = templateType == "user" and self:GetDBValue("global", "templates")[strupper(templateName)]
		or self.templates[strupper(templateName)]
	...
	if saveOrder then
		...
		for buttonID, _ in self.pairs(template, function(a, b)
